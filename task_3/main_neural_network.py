...
X_train, X_val, y_train, y_val = train_test_split(train_mutant, train_labels, test_size=0.25, random_state=42)
...
model.fit(X_train, y_train, epochs=60, class_weight=weights, callbacks=[f1_metric],verbose=1)

#test_mutant = normalize(test_mutant, axis=1)
final_model = load_model('best_model.h5')

print('predictions')
test_labels = final_model.predict(test_mutant)

print('rounding')
test_labels = np.round(test_labels).reshape(-1)

print('write csv file')
np.savetxt('sample_neural.csv', test_labels, delimiter='\n', fmt ='%d')
